...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        # get user from the scodpe using the auth middleware
        user = self.scope["user"]
        if user.is_anonymous:
            await self.close(401)
            return
        # track current user channel in databse
        await update_user_channel(user, self.channel_name)
        # register the channel/consumer on all chat room groups the user has subscribed to
    ...
    async def receive(self, text_data):
        ...
        # to all channels residing in the message_room
        # we are certain the other users will recieve messages in real time
